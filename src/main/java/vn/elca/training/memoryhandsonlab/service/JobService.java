...
public class JobService implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        for (Pair<Long, Student> jsonRow : studentsByIdJsonData) {
            try {
                Student savedStudent = studentService.updateInfo(jsonRow, dbStudents, nbUpdatedStudents, nbNewStudents, curLineNumber++);
    ...
    private void saveAuditInfo(List<StudentAudit> studentAudits, Student savedStudent) {
        ...
        Long prevItemId = Optional.ofNullable(!audits.isEmpty() ? audits.iterator().next() : null)
                                  .map(StudentAudit::getId)
                                  .orElse(null);
        
        StudentAudit studentAudit = new StudentAudit(null, savedStudent.getId(), savedStudent.getName(), savedStudent.getPassportNumber(), new Date(), savedStudent.getAddress().getName(), prevItemId);
